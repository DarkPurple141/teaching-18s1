...
Node copy(Node head) {
    ...
    while (head) {
      list = list_append(list, create_node(head->data));
      head = head->next;
    }
...
static int ordered_helper(Node curr, int prev) {
   if (curr) {
      if (!(curr->data > prev)) {
         return 0;
      } else {
         return ordered_helper(curr->next, curr->data);
      }
   }
   return 1;
}
...
int ordered(Node head) {
   if (head) {
      return ordered_helper(head, head->data -1);
   } else {
      return 1;
   }
}
